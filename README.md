...
교통 상황 정보 표시 UI: 주요 도로의 실시간 교통 상황을 한눈에 파악할 수 있도록 시각적으로 구현했습니다. (API 연동)<br><br><br><br>readme 꾸미는거 어캐함?? 이거 우리 조 사람이 해논건데 자기꺼 올린거라 이걸 내가 내 정보로 수정해야 하거든?? 
내가 시작이라 하기전까지 내가 한거  ppt 보낼태니까 저기에 정보를 어떻게 변경해야 할지 너가 구상해봐 시작이라 해야 구상 시작하고 그전까지는 내가 보내는 내 ppt 보고 내가 어떤 부분을 맡았는지 분석을 스스로 하고있도록
